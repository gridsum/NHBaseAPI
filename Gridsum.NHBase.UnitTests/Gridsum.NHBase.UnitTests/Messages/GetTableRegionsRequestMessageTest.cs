...
{
	[TestFixture]
	class GetTableRegionsRequestMessageTest
	{
		#region Methods.
		...
		{
			GetTableRegionsRequestMessage req = new GetTableRegionsRequestMessage();
			...
			client.send_getTableRegions(Encoding.UTF8.GetBytes(req.TableName));
		...
		public void DeserializeTest()
		{
			...
			GetTableRegionsRequestMessage newObj;
			Assert.IsTrue(ThriftObjectEngine.TryGetObject<GetTableRegionsRequestMessage>(container, out newObj) == GetObjectResultTypes.Succeed);
			Assert.IsNotNull(newObj);
			Assert.AreEqual(newObj.TableName,req.TableName);
		}
